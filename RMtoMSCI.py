...
#CAD-USD X-RATE Curncy in Bloomberg for BD-1 as function argument    
    ...
    open(r'F:\Portia\JOH_INSIGNIS.{}.cntl'.format(title), 'a').close()
    ...
    newtext = []
    test = open(r'F:\Portia\TNC.JOH.PROD.YYYYMMDD-YYYYMMDD.txt').readlines()
    ...
    with open(r'F:\Portia\TNC.JOH.{}.txt'.format(title), 'w') as output:
        for item in newtext:
            output.write(item)  
        
    os.remove(r'F:\Portia\TNC.JOH.PROD.YYYYMMDD-YYYYMMDD.txt')
    ...
    newtext = []
    test = open(r'F:\Portia\HLD.JOH.PROD.YYYYMMDD-YYYYMMDD.txt').readlines()
    firstline = test[0]
    ...
    with open(r'F:\Portia\HLD.JOH.{}.txt'.format(title), 'w') as output:
        for item in newtext:
            output.write(item)
            
    os.remove(r'F:\Portia\HLD.JOH.PROD.YYYYMMDD-YYYYMMDD.txt')
    ...
    os.makedirs(newpath)
    move(r'F:\Portia\TNC.JOH.{}-{}.txt'.format(lb2, lb2), newpath) 
    move(r'F:\Portia\HLD.JOH.{}-{}.txt'.format(lb2, lb2), newpath) 
    move(r'F:\Portia\JOH_INSIGNIS.{}-{}.cntl'.format(lb2, lb2), newpath)
    
    oldpath = r'F:\Portia\RiskMetricsProd\{}'.format(lb3)
    copy2(oldpath+'\JOH.{}.meta.xml'.format(lb3), newpath)
    copy2(oldpath+'\JOH.{}.repcntl'.format(lb3), newpath)
    copy2(oldpath+'\PTR.JOH.{}-{}.txt'.format(lb3, lb3), newpath)
    
    os.rename(newpath+'\JOH.{}.meta.xml'.format(lb3), newpath+'\JOH.{}.meta.xml'.format(lb2))
    os.rename(newpath+'\JOH.{}.repcntl'.format(lb3), newpath+'\JOH.{}.repcntl'.format(lb2))
    os.rename(newpath+'\PTR.JOH.{}-{}.txt'.format(lb3, lb3), newpath+'\PTR.JOH.{}-{}.txt'.format(lb2, lb2))
